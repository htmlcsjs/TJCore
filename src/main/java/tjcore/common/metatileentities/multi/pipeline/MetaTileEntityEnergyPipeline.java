...
import static tjcore.api.pipeline.PipelineManager.PIPELINE.ENERGY;
import static tjcore.api.pipeline.PipelineManager.PIPELINE.FLUID;
import static tjcore.common.CommonProxy.PipelineManager;

public class MetaTileEntityEnergyPipeline extends MultiblockWithDisplayBase implements IPipeline {
    ...
    EnergyContainerList inputs;
    EnergyContainerList outputs;
    int pipelikeTotal;
    int pipelikeRequirement = -1;
    NotifiableItemStackHandler pipelikeInput = new NotifiableItemStackHandler(1, this, false);
    MetaTileEntityEnergyPipeline connection;

    public MetaTileEntityEnergyPipeline(ResourceLocation metaTileEntityId) {
    ...
    public void setConnection(IPipeline pipeline) {
        ...
        if (pipeline.getType() == getType()) {
            this.connection = (MetaTileEntityEnergyPipeline) pipeline;
            isConnected = true;
    ...
    public void disconnect(boolean recyclePipes) {
        ...
        if (recyclePipes) {
            pipelikeInput.insertItem(0, OreDictUnifier.get(OrePrefix.cableGtQuadruple, Aluminium, Math.min(Math.min(pipelikeTotal, pipelikeRequirement), 64)), false);
        }
    ...
    protected void formStructure(PatternMatchContext context) {
        ...
        outputs = new EnergyContainerList(getAbilities(OUTPUT_ENERGY));
        inputs = new EnergyContainerList(getAbilities(INPUT_ENERGY));
    }
    ...
    protected void updateFormedValid() {
        if (getOffsetTimer() % 20 == 0 && this.isSource && connectionValid && connection != null) {
            long toDrain = Math.min(inputs.getEnergyStored(), connection.outputs.getEnergyCanBeInserted());
            connection.outputs.addEnergy(toDrain);
            inputs.removeEnergy(toDrain);
        }
        if (!connectionValid && connection != null) {
            if (getOffsetTimer() % 5 == 0 && !pipelikeInput.getStackInSlot(0).isEmpty() && pipelikeRequirement > 0) {
                ...
                if (stack.isItemEqual(OreDictUnifier.get(OrePrefix.cableGtQuadruple, Aluminium))) {
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new MetaTileEntityEnergyPipeline(metaTileEntityId);
    }
